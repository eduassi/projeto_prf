...
class APS_Element {
  ...
  setup_flags() {
    ...
    let configs = $(this.html_element)
      .attr("animation-configuration")
      .split(" ");
    ...
    if (this.flags["looping"]) {
      let looping_counter = Math.abs(
        Number($(this.html_element).attr("looping-counter"))
      );
  ...
  async regular_animation() {
    ...
    while (
      self.flags["looping-counter"] == -1 ||
      counter < self.flags["looping-counter"]
    ) {
      for (
        var next_frame_number = 0;
        next_frame_number < self.frames.length;
        next_frame_number++
      ) {
      ...
      if (self.flags["palinmation"]) {
        for (
          var next_frame_number = self.frames.length - 2;
          next_frame_number >= 0;
          next_frame_number--
        ) {
          next_frame = self.frames[next_frame_number];
...
var change_scroll_position_by_top_object = function(
  element,
  offset_top,
  margin_top
) {
...
var refresh_objects_listeners = function() {
  ...
  $.each($(".ventanita"), function(index, element) {
    ...
    $.each(button_collection, function(default_index, button) {
      ...
      let target_screen_object =
        target_id != "#null" ? $(text_box).find(target_id) : $(default_id);
  ...
  function refresh_bar(text_items, bar, current, quantum, i) {
    ...
    bar_val =
      current == text_items.length - 1
        ? 100
        : Math.round(quantum + quantum * current);
  ...
  $.each($(".jesus_slider"), function(index, element) {
    ...
    forward_buttons.on("click", function() {
      if (current < ball_items.length) {
        let margin_offset =
          parseInt(jesus_head.css("margin-left")) - item_width - 32;
        margin_offset += "px";
        jesus_head.animate({
  ...
  $.each($(".jesus_slider_ball"), function(index, element) {
    ...
    buttons.each(function(index) {
      ...
      this_button.on("click", function() {
        ...
        let margin_offset =
          parseInt(jesus_head.css("margin-left")) +
          item_width * difference +
          32 * difference;
        margin_offset += "px";
        jesus_head.animate({
          marginLeft: margin_offset
  ...
  $.each($(".jesus_slider_no_arrow_image_background"), function(
    index,
    element
  ) {
    let current = 0;
  ...
  $.each($(".flipster_carousel_slider"), function(index, element) {
    let current = 0;
    let current_fog = $(this)
      .find(".flipster_carousel-head")
      .find(".fog");
    ...
    flipster_items.on("click", function(event) {
      ...
      if (current != order) {
        ...
        let jump_offset = order - current;
        let margin_offset =
          parseInt(flipster_head.css("margin-left")) -
          margin_rate * jump_offset;
  ...
  $.each($(".matchup"), function(index, element) {
    ...
    let switch_settings =
      window.innerWidth > 480 ? "settings" : "mobile_settings";
    ...
    let card_image_template =
      ...
      '>\
                            ...
                            <div class="matchup-card-back flip-card-back" ' +
      dimensions_string +
      '>\
    ...
    for (var r = 0; r < rows; r++) {
      ...
      for (var c = 0; c < columns; c++) {
        ...
        $(new_card)
          .find(".matchup-card-content")
          .attr("src", cards[card_index]["card"]);
        $(new_card)
          .find(".flip-card")
          .attr("position", card_index);
        ...
        if (!freeze_time) {
          if (!$(this).hasClass("rotate")) {
            ...
            if (open_cards.length >= 1) {
              ...
              let isCrush =
                cards[index_first_card]["id"] == cards[index_last_card]["id"];
  ...
  function fitin_piece(actualtarget, droptarget, result) {
    ...
    $(actualtarget).css({
      top: position_target.top,
      left: position_target.left
    });
  ...
  $.each($(".puzzle"), function(index, element) {
    ...
    let responsive_size =
      piece_height < piece_width ? piece_height : piece_width;
    ...
    let background_string =
      ...
      puzzle_image +
      "); background-size: " +
      responsive_max * factor +
      "px " +
      responsive_max * factor +
      "px";

    let piece_template =
      '<div class="puzzle-piece puzzle-dropping" style="' +
      dimensions_string +
      background_string +
      '"></div>';
    let puzzle_cell_template =
      '<div class="puzzle-cell puzzle-dropping" style="' +
      dimensions_string +
      '"></div>';
          ...
          drop: function(evt, droptarget) {
            ...
            if ($(droptarget).hasClass("puzzle-cell")) {
              ...
              if (pre_drag_cell && id_cell_target_piece) {
                // SWAP
                hand_piece_result = fitin_piece(
                  this,
                  $("#" + id_cell_target_piece)[0],
                  result
                );
                swapped_piece_result = fitin_piece(
                  droptarget,
                  $("#" + pre_drag_cell)[0],
                  result
                );
            ...
            false_counter +=
              former_boolean - (hand_piece_result + swapped_piece_result);
            if (false_counter == 0) {
              for (var i in pieces) {
    ...
    for (var index = 0; index < pieces.length; index++) {
      ...
      let random_top_offset =
        (random_top_seed < 0.5 ? -random_top_seed : random_top_seed) * 10;
      let random_left_offset =
        (random_left_seed < 0.5 ? -random_left_seed : random_left_seed) * 10;
  ...
  function pick_another_question(
    lss_database,
    current_question,
    title_container,
    choices_container
    ...
    for (var i = 0; i < lss_settings["answers"]; i++) {
      ...
      $(input_answer).on("click", function() {
        if (!processing_next_round) {
          $("html, body").animate(
            { scrollTop: game_box[0].offsetTop - offset_top - margin_top },
            200
          );
          $(question_box).hide();
          $(julia_waiting).hide();
          ...
          aps
            ...
            .then(function() {
              ...
              if (current_question < max_questions) {
                round_answers = pick_another_question(
                  lss_database,
                  current_question,
                  title_container,
                  choices_container
                ...
                $(progress_led).css({
                  width: (current_question + 1) * quantum_question + "%"
                });
              } else {
